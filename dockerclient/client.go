...
// BuildStage contains all information needed for the current stage of the build.
...
type ClientExecutor struct {
	...
	// Stages slice holds all BuildStage's that make up the build.
	Stages []*BuildStage
}

// NewClientExecutor creates a *ClientExecutor.
...
// AddStage adds a new *BuildStage to the current *ClientExecutor.
func (e *ClientExecutor) AddStage(client *docker.Client) {
	e.Stages = append(e.Stages, &BuildStage{
		Client:                  client,
		ContainerTransientMount: "/.imagebuilder-transient-mount",
	})
}

// Stage returns the *BuildStage that's currently being processed.
// It's guaranteed that there's at least one stage.
func (e *ClientExecutor) Stage() (*BuildStage, error) {
	l := len(e.Stages)
	if l == 0 {
		return nil, fmt.Errorf("build has not stages at the moment")
	}
...
func (e *ClientExecutor) Prepare(b *imagebuilder.Builder, node *parser.Node, from string) error {
	...
	if s.Image == nil || e.isMultiStage() {
		if from == imagebuilder.NoBaseImageSpecifier {
			...
			e.Deferred = append(e.Deferred, func() error { return s.Client.RemoveImage(from) })
		}
		//log.Printf("AQUI: %s", e.Image)
		glog.V(4).Infof("Retrieving image %q", from)
		s.Image, err = e.LoadImage(from)
		if err != nil {
			return err
		}
	}
	...
	// statements
	if err := b.FromImage(s.Image, node); err != nil {
		return err
	}
	...
	if s.Container == nil || e.isMultiStage() {
		...
		if mustStart || e.isMultiStage() {
			...
			if len(e.TransientMounts) > 0 {
				...
				v, err := s.Client.CreateVolume(docker.CreateVolumeOptions{Name: volumeName})
				if err != nil {
					return fmt.Errorf("unable to create volume to mount secrets: %v", err)
				}
				e.Deferred = append(e.Deferred, func() error { return s.Client.RemoveVolume(volumeName) })
				sharedMount = v.Mountpoint
				opts.HostConfig = &docker.HostConfig{
					Binds: []string{volumeName + ":" + s.ContainerTransientMount},
				}
			}

			// TODO: windows support
			if len(s.Command) > 0 {
				opts.Config.Cmd = s.Command
...
func (e *ClientExecutor) Commit(b *imagebuilder.Builder) error {
	...
	if s.Container.State.Running {
		glog.V(4).Infof("Stopping container %s ...", s.Container.ID)
		if err := s.Client.StopContainer(s.Container.ID, 0); err != nil {
			return fmt.Errorf("unable to stop build container: %v", err)
		}
		s.Container.State.Running = false
	...
	if len(e.Tag) > 0 {
		...
		glog.V(4).Infof("Committing built container %s as image %q: %#v", s.Container.ID, e.Tag, config)
		if e.LogFn != nil {
			e.LogFn("Committing changes to %s ...", e.Tag)
		}
	} else {
		glog.V(4).Infof("Committing built container %s: %#v", s.Container.ID, config)
		if e.LogFn != nil {
	...
	image, err := s.Client.CommitContainer(docker.CommitContainerOptions{
		Author:     b.Author,
	...
	s.Image = image
	glog.V(4).Infof("Committed %s to %s", s.Container.ID, image.ID)

	if len(e.Tag) > 0 {
		for _, t := range e.AdditionalTags {
			repository, tag := docker.ParseRepositoryTag(t)
			err := s.Client.TagImage(image.ID, docker.TagImageOptions{
				Repo: repository,
				Tag:  tag,
			})
			if err != nil {
				e.Deferred = append(e.Deferred, func() error {
					return s.Client.RemoveImageExtended(image.ID, docker.RemoveImageOptions{Force: true})
				})
				return fmt.Errorf("unable to tag %q: %v", t, err)
			}
			e.LogFn("Tagged as %s", t)
		}
	}
...
func (e *ClientExecutor) PopulateTransientMounts(opts docker.CreateContainerOptions, transientMounts []Mount, sharedMount string) ([]string, error) {
	...
	for i, mount := range transientMounts {
		...
		copies = append(copies, imagebuilder.Copy{
			FromFS: true,
			Src:    []string{source},
			Dest:   filepath.Join(s.ContainerTransientMount, strconv.Itoa(i)),
		})
	}
...
func (e *ClientExecutor) CreateScratchImage() (string, error) {
	...
	return name, s.Client.ImportImage(docker.ImportImageOptions{
		Repository:  name,
		Source:      "-",
		InputStream: buf,
	})
...
func (e *ClientExecutor) LoadImage(from string) (*docker.Image, error) {
	...
	image, err := s.Client.InspectImage(from)
	if err == nil {
		return image, nil
	}
	...
	for _, config := range auth {
		...
		if err = s.Client.PullImage(pullImageOptions, authConfig); err == nil {
			break
		}
		lastErr = err
		continue
	...
	return s.Client.InspectImage(from)
}
...
func (e *ClientExecutor) Preserve(path string) error {
	...
	if s.Volumes == nil {
		s.Volumes = NewContainerVolumeTracker()
	}
	s.Volumes.Add(path)
	return nil
}
...
func (e *ClientExecutor) Run(run imagebuilder.Run, config docker.Config) error {
	...
	if e.StrictVolumeOwnership && !s.Volumes.Empty() {
		return fmt.Errorf("a RUN command was executed after a VOLUME command, which may result in ownership information being lost")
	}
	if err := s.Volumes.Save(s.Container.ID, e.TempDir, s.Client); err != nil {
		return err
	}

	config.Cmd = args
	glog.V(4).Infof("Running %v inside of %s as user %s", config.Cmd, s.Container.ID, config.User)
	...
	if err := s.Client.StartExec(exec.ID, docker.StartExecOptions{
		OutputStream: e.Out,
	...
	if err := s.Volumes.Restore(s.Container.ID, s.Client); err != nil {
		return err
	}
...
func (e *ClientExecutor) Copy(excludes []string, copies ...imagebuilder.Copy) error {
	...
	for _, copy := range copies {
		s.Volumes.Invalidate(copy.Dest)
	}
	...
	for _, copy := range copies {
		if copy.FromStage != "" {
			...
			if i < 0 || int(i) >= len(e.Stages) {
				return fmt.Errorf("--from value is out of range")
			}
			...
			err = s.Client.UploadToContainer(s.Container.ID, docker.UploadToContainerOptions{
				InputStream: ff,
				Path:        copy.Dest,
			})
			ff.Close()
	...
	return e.CopyContainer(s.Container, excludes, copies...)
}
...
func (e *ClientExecutor) CopyContainer(container *docker.Container, excludes []string, copies ...imagebuilder.Copy) error {
	...
	for _, c := range copies {
		...
		for _, src := range c.Src {
			...
			err = s.Client.UploadToContainer(container.ID, docker.UploadToContainerOptions{
				InputStream: r,
				Path:        "/",
			})
